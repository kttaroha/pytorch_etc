...
def get_distributed_dataloader(rank, world_size):
    ...
    train_dataloader = DataLoader(
        ...
        num_workers=0, drop_last=False, shuffle=False, sampler=train_sampler)
...
def eval_model(model, rank, test_dataloader, loss_fn):
    model.eval()
    num_correct = 0
    loss_test = 0.0
...
def train_model(rank, world_size):
    ...
    train_dataloader, test_dataloader = get_distributed_dataloader(rank, world_size)
    ...
    optimizer = optim.SGD(ddp_model.parameters(), lr=3*0.001)
    ...
    for n_epoch in range(N_EPOCHS):      
        ...
        if rank == 0:
            accuracy_test, loss_test = eval_model(model, rank, test_dataloader, loss_fn)
        ...
        print(current_time-start_time, n_epoch, accuracy_test)
